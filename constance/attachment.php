        ...
        <div id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
            ...
            <div class="entry-utility">
                <?php
                    printf( '<span class="%1$s">%2$s</span> %3$s',
						...
						esc_html__('By', "constance"),
                        sprintf( '<span class="author vcard"><a class="url fn n" href="%1$s" title="%2$s">%3$s</a></span>',
                            get_author_posts_url( get_the_author_meta( 'ID' ) ),
                            sprintf( esc_attr__( 'View all posts by %s', "constance" ), get_the_author() ),
                            get_the_author()
                        )
                    );
                ?>
                ...
                <?php
                    printf( '<span class="%1$s">%2$s</span> %3$s',
						...
						esc_html__('Published', "constance"),
                    ...
                    if ( wp_attachment_is_image() ) {
                        ...
                        printf( __( 'Full size is %s pixels', "constance" ),
                            sprintf( '<a href="%1$s" title="%2$s">%3$s &times; %4$s</a>',
                                wp_get_attachment_url(),
                                esc_attr__('Link to full-size image', "constance" ),
                                $nvr_metadata['width'],
                                $nvr_metadata['height']
                            )
                        );
            ...
            <div class="entry-content">
                <div class="entry-attachment">
                    ...
                    $nvr_attachment_size = apply_filters( 'constance_attachment_size', 940 );
