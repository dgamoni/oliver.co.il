...
;jQuery(document).ready(function(){
	 ...
	 */
	;jQuery('#update-nav-menu .button-primary, .nvr_theme_options_form .button-primary').click( function( event ) {
 ...
 */
;function nvr_generator_bind(){
	nvr_multiplier_numerator();
	nvr_multiplier_helpers();
	nvr_gradient_example();
	nvr_file_upload();
	nvr_dependency_watcher();
};

/* 
 * Function add one more item from example
 */
;function nvr_multiplier(){
	var nvr_clone = '';
	jQuery( '.multiplier_type' ).each( function( p_index, p_element ) {
		...
		jQuery( p_element ).find( '.multipler_add_one_more' ).click( function() {
			nvr_clone = jQuery( p_element ).find( '.multiplied_example > *' ).clone();
			jQuery( p_element ).find('.multiplied_content').append( nvr_clone );
			nvr_generator_bind();
		});
	});
};
...
/* 
 * Create helpers for nvr_multiplier.
 */
;function nvr_multiplier_helpers(){
	jQuery( '.multiplier_type' ).each( function( p_index, p_element ) {
 ...
 */
;function nvr_multiplier_numerator () {
	jQuery( '.multiplier_type' ).each( function( p_index, p_element ) {
		classes_new_id = 1;
		jQuery( p_element ).find( '.multiplied_content > div' ).each( function( index, element ) {
				...
				// replace all data-nvr_icon="" identificators
				jQuery(this).find('*[data-nvr_icon]').each(function(index,element){
					element_data = jQuery(element).attr( 'data-nvr_icon' );
					jQuery(element).attr( 'data-nvr_icon', element_data.replace( /\d+/g, classes_new_id ) );
				});
 ...
 * 
 */
;function nvr_dependency_watcher () {
